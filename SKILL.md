...
- Show only the oversized head emerging from the assigned lower-left or lower-right corner and filling about `85–95%` of the canvas. The head silhouette must physically cross both the bottom edge and the assigned side edge, occupying that corner; the crop is intentional.
- Make the crop an extreme close-up: the crown rises into the upper third of the square, and the opposite cheek approaches the opposite side with only a narrow background gap. The visible head should dominate the image rather than sharing equal visual weight with the background.
...
Composition: use an extreme close-up of the oversized head, upright and emerging from the assigned <lower-left or lower-right>, filling 85–95% of the square. Make the head silhouette cross both the bottom edge and the assigned side edge so it visibly occupies that corner. Raise the crown into the upper third and bring the opposite cheek close to the opposite side, leaving only a narrow background gap there. Preserve both paired identifying features. Never center or bottom-center the head, never float it above the bottom edge, and never leave background margin around all sides.
